...
state when available (`byResolution` or `byDisplayId`). The script maps that
anchor into the coordinate space of the monitor containing the live overlay.
The repair region is then centered on the mascot's standard 112x121 box with
monitor-relative padding. This matters because Windows `SetWindowRgn` limits
both mouse input and drawing; the region must cover the whole mascot to avoid
clipping the image.
